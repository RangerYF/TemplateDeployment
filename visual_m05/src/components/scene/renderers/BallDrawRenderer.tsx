...
import { ChartAxes, YGrid, ChartTitle, MT, PH } from '@/utils/svgChartUtils';
import { TrialDataTable } from './TrialDataTable';

const VW = 1040, VH = 500;
const ML = 68, MR = 260;
const PW = VW - ML - MR;
...
export function BallDrawRenderer({ result, replace, displayN }: { result: BallDrawResult; replace: boolean; displayN?: number }) {
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${VW} ${VH}`} preserveAspectRatio="xMidYMid meet">
      ...
      <ChartTitle title={`摸球结果分布 (${mode}, n=${displayedN})`} vw={VW} ml={ML} mr={MR} mt={MT} />
      ...
      <TrialDataTable
        ...
        rows={frequencies.map((freq, k) => ({
          label: `取到 ${k} 个红球`,
          count: Math.round(freq * displayedN),
          freq,
          theoreticalProb: result.theoreticalProbs[k],
        }))}
      />
    </svg>
  );
