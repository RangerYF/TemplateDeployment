...
export const SIM_DETAILS: Record<SimulationType, { title: string; detail: string; realWorld?: string }> = {
  ...
  pieChart: {
    title: '扇形统计图',
    detail: '将连续数据按组距划分为若干区间后绘制扇形图。扇区面积与该区间占总体的比例成正比，能够直观显示各部分相对整体的占比关系。',
    realWorld: '生活应用：选举得票占比、班级成绩等级分布、家庭支出结构分析。',
  },
  lineChart: {
    title: '折线统计图',
    detail: '将数据按序号展开为折线，便于观察数据随时间或样本编号的整体趋势。叠加均值参考线和最小二乘趋势线后可量化变化方向。',
    realWorld: '生活应用：股价走势、月度气温变化、班级考试均分逐次比较。',
  },
  tournamentMatch: {
    title: '三人羽毛球赛（累负淘汰）',
    detail: '模拟"累计负两场被淘汰、负者下场轮空"的三人轮空赛制（源自 2020 新高考Ⅰ卷题型）。可自定义三方两两胜率，跟踪指定事件并对照理论概率。',
    realWorld: '生活应用：双败淘汰赛制、循环赛轮空规则、各类多人竞技赛事的概率分析。',
  },
  boxSwapBalls: {
    title: '双盒互换球',
    detail: '甲乙两盒各装若干黑球与红球，每次操作各取一球后互相放入对方盒中。重复 n 次后统计甲盒黑球数的概率分布；可对照马尔可夫链稳态分布与递推公式。',
    realWorld: '生活应用：分子扩散模型、群体交换演化的简化模型；本类题型为新高考概率多选题常见模型。',
  },
  randomWalk1D: {
    title: '一维随机游走',
    detail: '每步以概率 p 向右、1-p 向左走一步。n 步后位置 X = (右移次数 - 左移次数)，其分布与二项分布 B(n,p) 一一对应。E[X]=n(2p-1)，σ=2√(np(1-p))。',
    realWorld: '生活应用：醉汉问题、股价短期波动、扩散模型；当 p=0.5 时为对称随机游走，是布朗运动的离散原型。',
  },
  randomWalk2D: {
    title: '二维随机游走',
    detail: '在平面格点上每步等概率向上下左右走一步。当 n 较大时，终点离起点的距离 √(x²+y²) 渐近服从 Rayleigh 分布，均值 ≈ √(πn/2)。',
    realWorld: '生活应用：动物觅食路径、网络爬虫遍历、分子扩散；演示"大量随机步组合成宏观规律"的核心思想。',
  },
  markovChain: {
    title: '通用马尔可夫链模拟器',
    detail: '老师可自定义状态空间 + 转移矩阵 + 初始分布，系统会模拟 n 条路径并对照理论稳态分布。覆盖天气链、传球链、信息传递链、心情链等所有高考题型。稳态分布 π 满足 πP = π。',
    realWorld: '应用：天气预测、用户行为转移、金融市场状态、PageRank 算法都是马尔可夫链。',
  },
};
...
export const PARAM_DESCRIPTIONS: Record<SimulationType, string[]> = {
  ...
  pieChart: [
    '数据来源 — 预设数据集或自定义数据',
    '分组数 — 将数据划分为多少个区间',
    '按频数降序 — 是否按各扇区频数从大到小重新排列',
  ],
  lineChart: [
    '数据来源 — 预设数据集或自定义数据',
    '显示趋势线 — 是否叠加最小二乘拟合的线性趋势',
    '显示数据点 — 是否在折线上绘制数据点标记',
  ],
  tournamentMatch: [
    'n — 模拟轮数',
    '甲胜乙 / 甲胜丙 / 乙胜丙 — 各对决的胜率（默认 0.5 公平赛）',
    '追踪事件 — 多选要统计的事件，对照理论概率（仅公平赛）',
  ],
  boxSwapBalls: [
    'n — 模拟轮数',
    '每盒初始黑球 / 红球 — 双盒初始构成（默认 1黑+2红）',
    '操作次数 — 每轮做几次互换操作（n=1, 2, 3...）',
  ],
  randomWalk1D: [
    'n — 模拟次数（统计终点分布）',
    '步数 — 每条路径包含的步数',
    '向右概率 p — 每步向右的概率（默认 0.5 为对称）',
    '展示路径数 — 同时绘制几条样本路径',
  ],
  randomWalk2D: [
    'n — 模拟次数（统计终点距离分布）',
  ...
  markovChain: [
    ...
    '演化步数 — 每条路径走多少步',
    '状态 — 可自定义状态名（2-6 个）',
    '转移矩阵 — P[i][j] = 从状态 i 到状态 j 的概率（每行和应为 1）',
    '初始分布 — 第 0 步各状态的初始概率（和应为 1）',
    '预设场景 — 内置天气链 / 三人传球 / 心情链 / 二态链',
  ],
};
