...
/** Build SVG polyline points string from array, with downsampling */
export function toPolylinePoints(
  data: number[],
  xMapper: (i: number) => number,
  yMapper: (v: number) => number,
  maxPoints = 500,
): string {
  const step = Math.max(1, Math.floor(data.length / maxPoints));
  const parts: string[] = [];
  for (let i = 0; i < data.length; i += step) {
    parts.push(`${xMapper(i).toFixed(1)},${yMapper(data[i]).toFixed(1)}`);
  }
  return parts.join(' ');
}
...
export function ChartAxes({
  ...
  return (
    <>
      ...
      {/* X label */}
      {xLabel && (
        <text x={ml + pw / 2} y={vh - 4} textAnchor="middle" fontSize={12} fill={COLORS.textSecondary}>
          {xLabel}
        </text>
      )}
      {/* Y label (rotated) */}
      ...
      {Array.from({ length: tickCount + 1 }, (_, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={ml - 6} y={y + 4} textAnchor="end" fontSize={11} fill={COLORS.textMuted}>
              {format(v)}
            </text>
          </g>
        );
...
export function ChartTitle({ title, vw = VW, ml = ML, mr = MR, mt = MT }: {
  ...
  return (
    <text
      ...
      textAnchor="middle" fontSize={14} fontWeight="bold" fill={COLORS.text}
    >
      {title}
    </text>
  );
