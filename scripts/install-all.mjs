...
import { detectPkgManager, installCmd } from "./utils.mjs";
...
async function runInstall(app) {
  ...
  console.log(`[install] ${app.id} (${pm}) -> ${cwd}`);
  await runInstallCommand(cmd, cwd);
}

for (const app of selected) {
  await runInstall(app);
}

console.log("全部模板依赖安装完成");
