...
export function renderTrigAnalysisOverlay(
  ...
  if (fnType === 'tan') {
    ...
    if (display.showSymmetryCenters) {
      drawPointMarker(ctx, viewport, centerX, transform.k, COLORS.angleArc, '中心');
    }
    if (display.showZeros && Math.abs(transform.k) < 1e-10) {
      const period = Math.PI / transform.omega;
      for (let n = -6; n <= 6; n++) {
        drawPointMarker(ctx, viewport, centerX + n * period, 0, COLORS.primary, `零点 ${formatPiLabel(centerX + n * period)}`);
      }
    }
    return;
  }
  ...
  if (display.showZeros && Math.abs(transform.k) < 1e-10) {
    for (let n = -6; n <= 6; n++) {
      ...
      drawPointMarker(ctx, viewport, x, 0, COLORS.primary, '零');
    }
  }

  if (display.showSymmetryAxes) {
    for (let n = -4; n <= 4; n++) {
      drawVerticalMarker(ctx, viewport, axis1 + n * period, COLORS.sinColor, '轴');
      drawVerticalMarker(ctx, viewport, axis2 + n * period, COLORS.tanColor, '轴');
    }
  }

  if (display.showSymmetryCenters) {
    for (let n = -4; n <= 4; n++) {
      drawPointMarker(ctx, viewport, centerBase + n * period / 2, transform.k, COLORS.angleArc, '中心');
    }
  }
}
